...
.anno_factors <- function (x, ids, which, type = c("row", "column")) 
{
  ...
  pal <-  c("#009E73","#E69F00", "#56B4E9", "#F0E442","#D55E00","#0072B2", "#CC79A7")
...
check_args_plotDiffHeatmapCustom <- function(u){
  ...
  stopifnot(is.data.frame(u$y) || is(u$y, "DFrame"), length(u$fdr_pal) == 
            ...
            length(u$row_anno) == 1, is.logical(u$col_anno) && length(u$col_anno) == 
              1 || CATALYST:::.check_cd_factor(u$x, u$col_anno, NULL))
}
...
plotDiffHeatmapCustom <- function (x, y, k = NULL, top_n = 20, fdr = 0.05, lfc = 1, all = FALSE, eff_r = NULL,
                                   ...
                                   row_anno = TRUE, hm_pal = NULL, fdr_pal = c("#C56B6B", 
                                                                               "#72DC72", "grey"), lfc_pal = c("blue3", "white", "red3")) 
{
  fun <- match.arg(fun)
